...
{
	[Export] public int Size { get; set; } = 50;

	[Export] public PackedScene Ammunition { get; set; }

	[Export] public PackedScene Egg { get; set; }

	[Export] public PackedScene Oxygen { get; set; }

	[Export] public PackedScene Food { get; set; }
	...
	public override void _Ready()
	{
		...
		void PlaceStreet(Vector2I areaPosition)
		{
			for (var x = 0; x < Areas.Size.X; x++)
			{
				...
				{
					var tilePosition =
						new Vector2I(areaPosition.X * Areas.Size.X + x, areaPosition.Y * Areas.Size.Y + y) + offset;
					...
					if (isAtEdge && !isAtCorner)
					{
						...
						if ((x == center.X || y == center.Y) &&
							map.GetArea(adjacentAreaPosition.X, adjacentAreaPosition.Y) == Area.House)
						{
							SetCell(terrainLayer, tilePosition, Terrain.Asphalt, rng);
		...
		{
			var hasPatrol = rng.RandiRange(0, 100) < 5;
			Faction faction = Enemies.GetFaction(rng);
			
			var maxEnemies = rng.RandiRange(1, 3);
			
			// No enemies in spawn area
			if ((areaPosition - new Vector2I(map.Size / 2, map.Size / 2)).Length() < 2)
			{
				hasPatrol = false;
			}
			...
			for (var x = 0; x < Areas.Size.X; x++)
			{
				for (var y = 0; y < Areas.Size.Y; y++)
				{
					...
					if (hasPatrol && maxEnemies > 0 && rng.RandiRange(0, 100) < 5)
					{
						PlaceObject(objectLayer, Enemies.GetEnemy(faction), tilePosition);
						maxEnemies--;
					}
				}
			}
		...
		void PlaceHouse(Vector2I areaPosition)
		{
			...
			if (roll < 50) roomType = RoomType.Loot;
			else if (roll < 80) roomType = RoomType.Stronghold;
			else roomType = RoomType.Empty;
			
			// No strongholds in spawn area
			if ((areaPosition - new Vector2I(map.Size / 2, map.Size / 2)).Length() < 2)
			{
				roomType = RoomType.Loot;
			}
			
			var hasAtLeastOneEnemy = false;
			...
			for (var x = 0; x < Areas.Size.X; x++)
			{
				for (var y = 0; y < Areas.Size.Y; y++)
				{
					...
					switch (roomType)
					{
						...
						case RoomType.Loot:
						{
							if (rng.RandiRange(0, 100) < 12)
							{
								PlaceObject(objectLayer, Ammunition, tilePosition);
							}
							else if (rng.RandiRange(0, 100) < 7)
							{
								PlaceObject(objectLayer, Food, tilePosition);
							}
						...
						case RoomType.Stronghold:
						{
							if (!hasAtLeastOneEnemy || rng.RandiRange(0, 100) < 20)
							{
								hasAtLeastOneEnemy = true;
								PlaceObject(objectLayer, Enemies.GetEnemy(faction), tilePosition);
							} 
							else if (rng.RandiRange(0, 100) < 30)
							{
								PlaceObject(objectLayer, Ammunition, tilePosition);
							}
							else if (rng.RandiRange(0, 100) < 15)
							{
								PlaceObject(objectLayer, Food, tilePosition);
							}
							else if (rng.RandiRange(0, 100) < 10)
							{
								PlaceObject(objectLayer, Oxygen, tilePosition);
							}
